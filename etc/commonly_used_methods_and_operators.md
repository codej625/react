...
1. spread (전개) 연산자
```
JavaScript의 전개 연산자인 spread operator는 배열이나 객체를 확장하여 그 안의 요소를 개별적으로 추출하는 데 사용된다.
(깊은 복사가 가능하다.)
```
```javascript
/* 배열 복사하기 */
...
```
filter 메서드는 주어진 함수의 조건을 만족하는 요소들로 이루어진 새로운 배열을 생성한다.
```
```javascript
...
```
인덱스는 0부터 시작하며,
첫 번째 매개변수는 추출을 시작할 인덱스이고 두 번째 매개변수는 추출을 종료할 인덱스이다.(종료 인덱스는 포함하지 않음)
```
```javascript
...
5. map (반복)
```
배열의 각 요소에 대해 주어진 함수를 호출하고, 그 함수가 반환하는 값을 모아서 새로운 배열을 생성한다.
```
```javascript
